...
print(paste('E(XYZ) = ', result))
```

From the Metropolis Hastings Sampler, we estimate that $\mathbb E (XYZ) = 0.087152477155928$ with burn-in period set to 500 and number of iterations set to 10,000. We use the formula $\frac{ \sum_{i = 501} ^{10,000} X_i Y_i Z_i}{10,000 - 500}$.
...
From the Gibbs Sampler, we estimate that $\mathbb{E}(XYZ) = 0.08905299$, with initial point at $(1,1,1)$ using the same burn-in period and formula from part (a). This is very similar to the result from the Metropolis-Hastings Sampler, so we are satisfied.
